...
rather than having to focus on handling provides a
lot of support for implementing protocols and helper classes etc. Also as I am using a state
machine
...
very fast code, and gives a lot of control over the behaviour of the whole system. As a systems
programming language, it has a lot of potential as for a distributed system. However it is
very verbose, and as I need to iterate . Another option was erlang. Erlang was designed for networking and exhibits a high
...
message containing the $\varphi_2$, $p_2$ can update its current proposal number to $\varphi_2 + 1$, and
not even have to wait for one timeout period before receiving a \msg{Promise} message. A
...
With this extension, the protocol still has protection from dropped or lost messages (the safety
and liveness properties are still preserved), but in the case of a crashed Proposer, the recovery
...
Proposer mixin is the most complicated
state machine, 3 states
starts in idle
goes to trying, sends prepare messages
schedules timeout check
when received majority promises - set status to polling
checks whether anyone already requested value
if yes - restart instance (can also not care, and just want to find out - NOP if no-one did but
don't bother restarting if they did)
if no - request our value

\subsubsection{Acceptor}

acceptor is a fairly simple (not even state machine?)
just records variables and sends messages

\subsubsection{Learner}

learner is simple
records a set for each prop num seen (per instance).
...
% XXX: diagram of read showing updated op log
