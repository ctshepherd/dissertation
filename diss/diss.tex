...
The aim of the project was to implement a distributed database, based on the Paxos protocol.
The Paxos protocol must be implemented in a module, providing an interface for the database to
use. The database must have all ACID properties and implement a subset of SQL.

\section*{Work Completed}

Paxos was implemented in full and an ACID distributed database with support for transactions was
implemented on top of it. Several protocol improvements were implemented and the performance
implications of using these were quantitatively evaluated.
...
Consensus in a distributed system is a difficult problem. The consensus problem is that of getting
all nodes in a distributed system to agree on a value. Multiple protocols have been
...
While attempting to disprove the existence of a protocol with the safety properties that Paxos provides,
Lamport instead ended up creating Paxos. Despite his assertion in \emph{Paxos Made Simple}
\cite{lamport01} that the protocol is relatively simple, it has a reputation for being hard to implement.  With this in mind, I set out
to build a distributed database based on the protocol.  I took on the challenge of not only
implementing it, but also designing and coding a database layer on top of it. I wanted this database to
...
Formally, an algorithm that satisfies the
consensus problem satisfies the following three properties:

\begin{enumerate}
\item Agreement - all nodes must decide on the same value.
\item Validity - the value that is decided upon must have been proposed by a node in the
	network.
\item Termination - all nodes eventually decide on a value.
\end{enumerate}

Decision is defined as follows: a node must decide on a value only once, and cannot change the
value once it has been decided.

\subsection*{Failure Modes}

Distributed networks must be able to deal with several forms of failure. One model of failure is
the \emph{fail-stop} model, where nodes may fail but then stop executing completely. However in
practice, a more accurate model is the \emph{fail-recover} model. This is where a node stops
executing, then resumes execution at a later time. This is a more realistic model, where
asynchronous networks mean that it may take an arbitrarily long time to receive a message, and
that messages may arrive out of order or not at all. The most general failure model is the
\emph{Byzantine failure} model, where nodes may respond incorrectly or even deliberately lie in order to
mislead the protocol. In practice this is unlikely, and can be mediated using checksums, message
...
co-ordinator\footnote{The node initiating the protocol.} asks each node in the cohort\footnote{The
other nodes in the network.} to decide
where to commit the value it suggests or abort the transaction\footnote{I.e. this is
a binary decision. If a node wants a different value it must initiate its own
...
However, 2PC suffers from a lack of tolerance to failures. If the
co-ordinator fails during the protocol then some nodes in the cohort will have received a message from
...
nodes of the decision. However, if during that recovery a member of the cohort crashes,
...
3PC still suffers from a flaw, in that it cannot recover from a network partition. If the
network is partitioned such that all nodes in the cohort that are in the pre-commit phase are on one
side of the partition and all nodes that are not are on the other side, the first side of the
...
This is unfortunate, as computer networks are asynchronous networks and can be modelled
using a fail-recover model to account for arbitrary delays between messages being sent to being
...
However, a potential drawback to this is that Paxos sacrifices availability. This is
a pathological case where no progress is made, i.e., there is no upper bound on the
time it will take to achieve consensus. This is due to the FLP
impossibility proof \cite{fischer85}, which states that it is impossible to guarantee liveness in
an asynchronous network where at least one failure has occurred. In practice, it is a situation that
...
clients reading and writing the same data simultaneously. It also enables effective abstraction, as
the atomicity requirement in particular gives well defined characteristics when an operation
...
\subsection*{Comparison between centralised and distributed databases}
...
must conform to the description of the algorithm specified in \emph{The Part Time Parliament} \cite{lamport98}.
...
\paragraph{SQL} The interface to the database must be SQL. It may be a subset of SQL, as the
...
Implementation, Verification and Maintenance. I decided against using this and instead used the
...
The Spiral Model was more suitable for my project because, although formal
requirements for the project were set out, there was still a lot of uncertainty in the project. I
had not designed a network
protocol or a database previously, nor had I implemented Paxos before. This meant it was
likely I would encounter unexpected difficulties in developing my project that would lead me to
change or shift my requirements. In this case the Spiral Model is more flexible than the Waterfall
Model.

\subsection{Language Choice}

There are many languages this project could have been implemented in, but the two main choices were the
...
\begin{itemize}
\item Python handles memory allocation internally; whilst C provides great precision over memory
...
threaded, which means I need not worry about concurrency issues in my project. Twisted is
...
I chose Git as the version control system for my project, as it is the VCS I have the most
familiarity with. I also used it as a backup strategy, keeping everything related to
my project in the Git repository and updated two different servers with it every few hours of
work.

\subsection{Testing}

In order to have confidence that my project works correctly I wrote
tests. I wrote unit tests for the main modules of my project, to check it handles both expected and
malformed input correctly and as expected. Tests are also useful for
expressing the API usage of the different modules of the project and checking that the interfaces
connect sensibly. I also performed regression testing; every time I fixed a bug I
added a test ensuring that any regression causing the bug again would make the test fail.

Tests will be run every time I make a commit, to confirm that the project
is working correctly. Twisted has a unit testing framework that I used as it easily
...
the database can be confident that their transactions will be processed reliably. I will go
into some detail, here about what each of these properties mean.

\subsection*{Atomic}

Atomicity means that either an operation completes or it does not, there is no `halfway' state.
This ensure the programmer using the database does not need to worry about cleaning up after
an operation, because if it does not succeed they can simply retry, without needing to worry about
the state in which the database has been left. Atomicity also applies to transactions in the same way ---
...
were executed sequentially. It ensures that transactions that execute concurrently
...
\begin{enumerate}
\item The co-ordinator sends a \msg{PROPOSE} message to each
...
any messages, consensus is trivially satisfied, as the system cannot be in an inconsistent state
if nothing has happened. However, if the co-ordinator crashes after sending
...
a blocking protocol in an asynchronous network.
...
\item Each cohort member responds with a \msg{YES} or \msg{NO}. Again, if there is a failure or timeout the cohort
...
\item \msg{Confirm}: Once a cohort member has received a \msg{Finalise} message, it commits the
	transaction, even if the co-ordinator fails. It replies with a \msg{Confirm} message.
\end{enumerate}
...
Paxos is a distributed consensus algorithm. It is failure tolerant for up to $F$
simultaneous failures in a network of $2F + 1$ nodes.

Paxos is actually a family of protocols, based around the same main algorithm. This
...
\begin{itemize}
\item Integrity - every correct process decides on at most one value, and if it decides some value
...
\item Every correct process decides some value.
\end{itemize}
...
can tolerate this, albeit at a failure tolerance of $F$ for $3F + 1$ nodes.
This variant is not discussed further in this dissertation.
...
can be compared to any other proposal, allowing the maximum ordered proposal
to be found. The conventional way to achieve this is to define a proposal
...
\item \msg{Promise(n, m, v)}, promising not to accept proposals numbered less than
	$n$. $m$ is the number of the highest proposal previously accepted by the recipient and
...
roles - Proposer, Acceptor and Learner. Explaining it in terms of these roles is simpler, although
...
set becomes a quorum (i.e., the size of the set $S$ becomes greater than $\frac{N}{2}$, where $N$ is
the size of the network), the Learner can `learn' $v$ as the value decided on for that
...
case, the behaviour is reasonably straightforward to follow. There are four message delays
until the proposed value is learnt.

\begin{enumerate}
\item \msg{Prepare(n):} The Proposer sends a \msg{Prepare} message to the Acceptors.
\item \msg{Promise(n):} The Acceptors all promise not to accept proposals numbered $< n$, as they
...
\item \msg{Prepare(1):} The Proposer sends a \msg{Prepare} message to the Acceptors, but is only
	able to message one of them successfully.
...
\item \msg{AcceptRequest(2, b):} The second Proposer asks the Acceptors to accept \verb+'b'+ as the
	value.
\item \msg{AcceptNotify(2, b):} As the Acceptors make up a quorum the network is able to proceed to
	completion.
\end{enumerate}
...
for Paxos --- duelling Proposers. This is the pathological case for Paxos where it is possible
that
it never makes progress. However this eventuality is unlikely because the messages from the proposers
...
\item \msg{Prepare(1):} The Acceptors respond with a \msg{Promise} not to accept proposals
	numbered $< 1$.
\item \msg{Prepare(2):} Before the first Proposer sends an \msg{AcceptRequest} message, another
	Proposer sends a \msg{Prepare} message to the Acceptors asking them not to accept
	proposals $< 2$.
\item \msg{Promise(2):} The Acceptors promise they will not.
\item \msg{AcceptRequest(1, a):} The first Proposer sends an \msg{AcceptRequest} message to the
	Acceptors, but they cannot accept as they have promised not to accept proposals $< 2$.
...
Twisted provides the event loop and network interfaces, making it possible to listen on
an interface and have a callback be passed events that occur.

% \section{Development}
%
% % XXX: is this section alright?
% % XXX split into 2 sentences
% % XXX two "as"s
...
Figure~\ref{fig:module-layout} shows the basic design of the code, illustrated with how objects
...
I decided to use Python dictionaries as a message object, and serialised them to a plain string
...
Choosing to use strings as a message format over a binary format was important as my priority was
being able to develop efficiently and using a binary format would hinder my debugging, compared to
logging strings.  Optimising the message format for speed would be a premature optimisation and,
in a prototype implementation, speed was not my primary concern.

Although security was not a major concern for this project, I wanted to be able to serialise
...
\op{ATTEMPTLOCK} request. A more secure implementation would therefore have nodes generate a
...
architecture this can be any node, however in a different model it may be a specific node.
When a node $N$ connects to the bootstrap
node $B$, it sends an \msg{EHLO} message\footnote{EHLO is taken from SMTP: http://cr.yp.to/smtp/ehlo.html}.
$B$ replies with a \msg{Notify} message containing all the
hosts $B$ is aware of. $N$ then sends each of these in turn an \msg{EHLO} message, making each of them
aware of its presence, and getting a \msg{Notify} message from each of them. This is repeated until
there are no nodes it has not learned the existence of. $N$ is then fully integrated into the network.\footnote{
Note that this bootstrap method is $O(n^2)$ in the number of messages sent, where $n$ is the
number of nodes in the network. There are other ways to
do bootstrap that are more efficient in the number of messages sent, these are discussed
later on.}

\subsubsection{Heartbeat}

To identify when a node has left the network, upon initialisation each node starts a timer on a
...
instance has the necessary state saved in a dictionary known as the \emph{instance state
...
lost. In fact, the most basic implementation of Paxos simply ignores messages if they do not
...
with a proposal number $\varphi_1$. $A$ then receives a \msg{Prepare} message $M$ from Proposer
...
response. $p_2$ will know $A$ is still part of the network because it will still respond
...
discuss this in the Evaluation chapter.
% XXX discuss it!
...
argument. The callback parses the message serialisation into a dictionary and checks the
...
3 states: `idle', `trying' and `polling' (see
figure~\ref{fig:proposer-state-machine})\footnote{Pseudocode for the Proposer role is presented in
Appendix A.}. The
...
Proposal numbers are a tuple of the form \verb+(p, <GUID>)+, where \verb+p+ is an integer
representing the numerical component. They
...
the operation log and its value needs to be ascertained.

\subsubsection{Acceptor}

The Acceptor class only stores three variables in each instance\footnote{Pseudocode for the
Acceptor role is also presented in Appendix A.}:\\
\\
...
the efficiency of the network in the real world.
...
The main challenge for the database is then to decide on an order for these operations that is
...
with a particular operation. OIDs are integers and refer to indices of the operation log. To
...
became clear that it made more sense to delegate the responsibility for retrying to Paxos, as it
allowed a simpler interface between the two modules.
...
I chose to use a \op{NOP} operation rather than a \op{READ} operation to do a slow read on other
nodes, although they
would semantically be the same (as other nodes do not need to perform an action on a \op{READ}), as I felt a \op{NOP}
better represented the operation being sent (do nothing).

\begin{figure}[htb]
\centering
\subfigure[Performing a slow read]{
	\includegraphics[scale=0.5]{figs/op-log-slow-read-1.eps}
	\label{fig:op-log-slow-read-1}
}
\subfigure[After a slow read]{
	\includegraphics[scale=0.5]{figs/op-log-slow-read-2.eps}
	\label{fig:op-log-slow-read-2}
}
\end{figure}
...
operation is inserted, the number of preceding \op{ATTEMPTLOCK}s is well-bracketed\footnote{There
...
metrics. These were designed to measure how the system performance differed with regard to
...
tests between a large number of machines is difficult, and the resources required were unavailable.
...
It became necessary to run all nodes in the same script in order to emulate more complex networks. I
discuss the limitations of this in the section below.
...
number of nodes increased. The problem was exacerbated because Python is limited to using only
...
to be aware of every other node in the network. This start up time is not reflected in the
results. After the network bootstrapped, the required number of nodes were scheduled to write,
and the relevant metric was recorded in a CSV file.

Five measurements were taken for each point, the average and standard deviation were then calculated
...
I started off by measuring how the network responded to contention. This meant measuring how the
latency and throughput changed in response to the number of nodes which were writing in the
network.
...
\subfigure[Contention in a network of 10 Nodes]{
	\includegraphics[scale=0.75]{figs/lat_10.eps}
	\label{fig:lat-10}
}
\subfigure[Contention in a network of 20 Nodes] {
	\includegraphics[scale=0.75]{figs/lat_20.eps}
	\label{fig:lat-20}
}
\subfigure[Network size effects] {
...
processing overhead. This overhead is minor, as explained in the Implementation chapter, as the
...
network increases linearly with the number of writers in the network.

I repeated the simulation with a network of 20 nodes, shown in figure~\ref{fig:lat-20}.  This
shows again that increasing the number of concurrent writers in the system causes the latency
...
of nodes in the network. This appears to increase with $O(N^2)$, $N$ being the number of nodes in the
network.
...
\centering
\subfigure[Contention in a network of 10 Nodes]{
	\includegraphics[scale=0.75]{figs/thru_10.eps}
	\label{fig:thruput-10}
}
\subfigure[Contention in a network of 20 Nodes]{
...
Figure~\ref{fig:nack} shows how the different NACK implementations compare with each other in
...
Interestingly, there is not a clear difference between NACK `version 1' and not using NACKs. This
may be due to the `ping pong' effect mentioned in the Implementation chapter. Although NACKs would
reduce the amount of timeouts occuring, they may increase the contention between Proposers and the
...
\caption{\label{fig:start}Startup Cost Measurements}
\end{figure}
...
significantly decreased, but the increase as the number of nodes rises is linear, rather than
$O(N^2)$.
...
% XXX rewrite, sentence makes no sense
Another limitation of the protocol is that it is quite verbose.
...
% 
% If I was to do the project again, there are several potential changes that I would make.
% 
...
% % XXX why change it if it was reasonable
% % nick says wheres the change
...
% % XXX: nick says change?

\cleardoublepage
\chapter{Conclusion}

I implemented Paxos and built an ACID distributed database on top of it, with support for transactions.
I made some improvements to my design and measured quantatively how these affected the performance
of the database under different loads.
...
There are multiple opportunities that could be investigated that would allow for
...
Although I decided it was beyond the scope of my project, it would be been
valuable to investigate the security implications of my design decisions
and consider implementing Byzantine Paxos, which can tolerate
