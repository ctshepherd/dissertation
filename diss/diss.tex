...
I created a seperate program for each node, but for all but the most simple network scenarios it
...
big overhead to using transactions, as evidenced from the substationally reduced throughput,
...
complete solution would be to use a DHT or super-node hierachy.
...
use an agile programming strategy, rather than an interative one. A quote that summarises my
...
Unit tests became a problem as I progressed through the project.  They became a hinderance, as I
...
and tradeoffs that I measured and quantatively evaluated in the Evaluation Chapter, also
...
A large part of contempary database design involves ensuring that database operations persist
