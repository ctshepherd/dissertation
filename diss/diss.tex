...
developed as a solution it but one of the most important is the Paxos protocol.
...
design centralised databases are generally simpler to organise, edit, query and to backup. It is
...
example is Google using it for ``Chubby'', their  distributed lock service described in
\emph{Paxos Made Live}\cite{chandra07}. This underpins their BigTable distributed database which
is used across Google.

\begin{quote}
While Paxos can be described with a page of pseudo-code, our complete implementation contains
several thousand lines of C++ code
\end{quote}
is a representative quote from the paper showing how intricate implementing Paxos actually is.
The paper described the difficulty of producing a production-ready system in practice. Moving from
a theoretical perspective to a practical one presented several problems. These included an
imprecise specification of the system and faults Paxos cannot deal with.
...
\op{ATTEMPTLOCK} request. A secure implementation would have nodes generate a public/private
keypair and sign messages with the private key, so that they could be verified with the public
key. This would prevent nodes forging messages.
...
problematic as if the network develops inconsistencies these are much harder to resolve.
...
the upper layer to process the ``learned'' value and any other useful information in the state.
