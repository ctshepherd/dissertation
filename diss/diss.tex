...
Paxos is a generalisation of 2PC and 3PC that handles more failure modes. It can support multiple proposers, 
...
To initiate a round of Paxos, a node sends out a Prepare message to a quorum
of acceptors with a unique proposal number, higher than any it has sent out
before.
...
If there are no failures, then the behaviour is reasonably straightforward to
follow. There are four message delays until the proposed value is learnt.
...
In \emph{Paxos Made Simple} \cite{lamport01}, the actions of a node are split into three main roles - {\bf Proposer},
