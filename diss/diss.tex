...
I simulated the network on a single machine. I did this for a number of reasons.
...
also makes results inconsistent and difficult to reproduce on several runs. For each data point in
my tests I calculated 5 runs and then calculated the average and standard deviation for each
point.
...
being CPU bound. However, as I am simulating a network on one computer, this assumption begins to
break down as the number of nodes increases. This problem is exacerbated because Python is
limited to only using one core without explicit parallelism, and Twisted's event loop is running
for not just one node but for all the nodes in the network. A way to fix this would be to spawn
...
However, in all cases we can see that the latency of commiting an operation to the
...
\caption{\label{fig:latency-rev}Rev Lat for 5 Nodes}
\end{figure}
...
In figure~\ref{fig:thruput-5} we can see that the throughput of the network decreases
exponentially with the number of concurrent writers in the system. We can also see that while
there is little overhead from using the database layer over the top of Paxos, there is substantial
overhead performing multiple transactions.
...
\caption{\label{fig:thruput-rev}Rev Thru for 5 Nodes}
\end{figure}
