...
Leslie Lamport - \emph{Paxos Made Simple} \cite{lamport01}
\end{myquote}
...
structuring the program flow around events
that occur. For a distributed application this mainly means in response to network packets
arriving, but can also be caused by user input or by timers firing.

Event Driven Programming is a paradigm that involves an event handling loop. Program flow is co-ordinated
...
that precedes it has been received, i.e., every operation before it is also available. The DBP Core
...
section.
due to event programming and way algorithm works

\paragraph{Phase 1}

% XXX: review
When an acceptor receives a message \msg{Prepare(n)} from a Proposer $P$, if $n > \verb|acceptor_prepare_prop_num|$
then it updates \verb|acceptor_prepare_prop_num| to $n$ and
% sends a reply of the form \msg{Promise(n, \verb+acceptor_cur_prop_num+, \verb+acceptor_cur_prop_value+)}.

\paragraph{Phase 2}

If an Acceptor receives a message \msg{AcceptRequest(n, v)}, if $n > \verb|acceptor_prepare_prop_num|$
it accepts the proposal. This involves setting $\verb|acceptor_cur_prop_num| := n$ and
$\verb|acceptor_cur_prop_value| := v$. It also notifies Learners in the network of its
decision by sending an \msg{AcceptNotify(n, v)} message to them.
...
\verb|handle_reject|, then called this method whenever a \msg{NACK} is received, thus initiating
...
number of \op{ATTEMPTLOCK}s is well-bracketed (i.e., there is an equal number of lock takes and
