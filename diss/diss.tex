...
% One optimisation we can perform that improves performance from the na\ddot{i}ve version is to
% collapse reads together. If the last operation was a read, and we want to read, we do not need to
% insert two entries in the transaction log. We still need to perform a round of Paxos to confirm
% that 
