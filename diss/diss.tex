...
Word Count:         & \bf 11,717 \\
Project Originator: & Charlie Shepherd                    \\
Supervisor:         & Stephen Cross                    \\
\end{tabular}
}

% XXX: note: cut down DVCS, cut bullshit about not having enough time, SQL etc
...
an asynchronous network where at least one failure has occured. In practice, it is a situation that
is relatively easy to avoid and is not a serious concern.

\section{Databases}

% XXX repeat im going to implement a database system
\subsection*{ACID}
...
protocol or a database previously, nor have I implemented Paxos before. This means that it is
...
In order to have confidence that my project works correctly I will need to write
tests. I will write unit tests for the main modules of my project, to check it handles both expected and
...
connect sensibly. I will perform regression testing; every time I fix a bug I will
% XXX bug twice (from dad)
...
% XXX: dad - this variant is not discussed futher in this disssertation
% XXX: go into this further
...
% XXX review nicks comments on the start of this chapter again
...
% XXX do i want to say its trivial?
...
% XXX rewrite? as per eds comments
...
became clear that it made more sense to delegate the responsibility for retrying to Paxos.
% XXX why?
...
% XXX say what: ed
I chose to use a \op{NOP} operation rather than a \op{READ} operation, although they
...
% XXX pyparsing didnt do this for you?
...
% XXX reword
It became necessary to run all nodes in the same script in order to control complex scenarios. I
...
% XXX does io bound need explaining?
...
network. Obviously the higher the contention the longer it would take operations to complete, but
by graphing the results  % XXX ...
...
Although I saw an improvement to the startup cost of bootstrapping the network, my improvement
centralises the bootstrapping. A more advanced solution would use a DHT or super-node
