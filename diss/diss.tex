...
Leslie Lamport - \emph{Paxos Made Simple}
\end{quote}
Despite this assertion, Paxos has a reputation for being hard to implement.

Both \emph{Paxos Made Live} \cite{chandra07} and \emph{Paxos Made Moderately Complex}
\cite{renesse11} repeatedly show how intricate implementing Paxos actually is.
...
2 Phase Commit is one of the simplest protocols that aims to solve the Distributed consensus
problem. The co-ordinator asks each node in the cohort to agree to commit the operation. If all
nodes agree then it asks each member to commit, if they do not all agree then it aborts the
operation.

The problem with the protocol is that it is a synchronous protocol. These means if a node fails
the co-ordinator is blocked waiting for them to respond.
% XXX

\subsection*{3 Phase Commit}

3 Phase Commit is a modification to 2 Phase Commit that adds another phase, splitting the commit
into pre-commit and finalise. This makes it possible to define all behaviour no matter what
node fails, and turns the protocol from a synchronous one into an asynchronous one.

However, 3PC also suffers from a flaw --- it cannot recover from network partition. If the network
is partition
...
It was developed by Leslie Lamport at Microsoft
research when he was trying to disprove its existence. 
...
$(n', v) \in M. \forall (n'', v'') \in M.  n' \ge n''$.
...
greater than $\frac{N}{2}$, where $N$ is the size of the network), the Learner can ``learn'' the $v$ as
